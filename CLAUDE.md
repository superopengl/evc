...
- **pro-components 3 removed `rightContentRender`.** Use `avatarProps` (as `AppLoggedIn` does for the user menu) or `actionsRender` (as `HomePage` does for the language switcher). Both land in the header's right-hand actions area. This fails *silently* - it cost the logout menu until it was spotted on screen.
- **`layout="side"` renders no header at all on desktop.** `DefaultHeader` bails on `layout === 'side' && !isMobile`, so `headerContentRender`, `avatarProps` and `actionsRender` fall through to the sider (or vanish). `AppLoggedIn` is `layout="mix"` for exactly this reason: it keeps the top bar (logo, symbol search, avatar menu) above the side menu. Side layout was fine in pro-layout 5, which is where the old `layout`-less config came from.
- **`navTheme` no longer themes the nav.** It only takes `'light' | 'realDark'`, and `'realDark'` darkens the *whole app*. `navTheme="dark"` is silently ignored. The dark sider is design tokens now - `proLayoutToken` in `src/antdTheme.js`, passed as `<ProLayout token={...}>`. Only the sider is dark; the header stays light, because the role-coloured avatar (`#00293d` for admin) disappears against a dark bar. One thing tokens cannot reach: the collapsed sub-menu flyout is portalled to `body` and ProLayout pins its `popupBg` to the light `colorBgElevated`, so it is tagged via `menuProps.classNames.popup.root` and coloured in `src/index.less`.
- **`List` is migrated to `Listy`** (antd 6 deprecates `List` and drops it next major). Listy is a much smaller primitive: `items` / `rowKey` / `itemRender` plus grouping and virtual scrolling, and that is all. There is no `List.Item`, `Item.Meta`, `header`, `footer`, `grid`, `pagination`, `loading`, `bordered`, `split`, `size`, `loadMore` or `locale.emptyText`. Per the migration guide the preset structures are recomposed as plain JSX inside `itemRender`; the avatar/title/description box appeared at eight call sites, so it lives once in `src/components/ListyItemMeta.jsx`. Three things to know when touching a migrated list:
  - **Listy owns the item element.** `itemRender` returns only the *content* - Listy wraps it in `.ant-listy-item` itself. Anything that used to sit on `List.Item` (`onClick`, `extra`, `style`) has to move into a wrapper inside `itemRender`, and per-item CSS moves to `styles.item`.
  - **The item box does not default the same way.** antd List used `12px 0` (`8px 0` at `size="small"`); Listy uses `12px 16px`. Every migrated list restates `paddingInline: 0` - miss it and the whole row gains a 16px inset. Listy also draws a divider under the *last* item, which antd List suppressed; `src/index.less` restores that with `.ant-listy-item:last-child`.
  - **`rowKey` is required and has no index fallback.** antd List quietly fell back to the row index, which hid at least one wrong key (`StockEpsAdminPanel` asked for `id`, but `StockEps` is keyed on `(symbol, reportDate)` and has no `id`). A bad `rowKey` now shows up as a React duplicate-key warning.
- **antd 6 removed the deprecated `visible` prop** on Modal/Drawer/Popover (use `open`), `onVisibleChange` on Tooltip/Popover (use `onOpenChange`), and `overlay` on Dropdown (use `menu`, or `popupRender` to keep an existing `<Menu>` element). All of these fail silently - no warning, the component simply never opens. 29 `visible` props and 2 `onVisibleChange` were converted.
- **Grepping for `visible={` does not find all of them.** Four wrappers took `visible` as their *own* prop and leaked it into antd through `{...props}` / `{...other}` - `AboutDrawer`, `EarnCommissionModal`, `ChangePasswordModal` - or passed it straight through under a styled alias (`CreditHistoryListDrawer`'s `styled(Drawer)`). The call site reads `<AboutDrawer visible={x}/>`, which looks like a custom prop, so the rename sweep skipped them and those four dialogs silently stopped opening. When auditing, check every component that spreads props into a Modal/Drawer, not just literal `<Modal visible=`.
- **Static `Modal.confirm/info/warning` do not see `<ConfigProvider theme>`.** They render into a detached React root, so antd warns "Static function can not consume context" and the buttons come out stock antd blue. `src/index.jsx` calls `ConfigProvider.config({ theme: antdTheme })` at bootstrap, which is the global theme those detached roots read - much cheaper than routing all 14 call sites through `App.useApp()`.
